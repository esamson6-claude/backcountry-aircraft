...
<script>
  ...
  // Favourites are stored per-browser and survive rebuilds, but listings come
  // and go — sold, withdrawn, filtered out. Count only the ones still on the
  // page, or the badge claims favourites that have no card to show. The stored
  // set is left intact on purpose: a listing that reappears keeps its heart.
  function liveFavCount() {{
    let n = 0;
    for (const u of favs) if (cardByUrl.has(u)) n++;
    return n;
  }}
  function updateFavBadge() {{
    const n = liveFavCount();
    favBadge.textContent = n;
    favBadge.style.display = n ? 'inline-block' : 'none';
  }}
  ...
  function apply() {{
    ...
    if (visible.length === 0) {{
      let msg = 'No listings match the current filters.';
      if (favView) {{
        // Distinguish "never saved anything" from "everything saved has since
        // left the site" — otherwise the empty page looks like a bug.
        msg = favs.size === 0
          ? 'No favorites yet — tap the ♥ on any listing to save it here.'
          : (liveFavCount() === 0
              ? 'Your saved listings are no longer on the site — they were sold or withdrawn.'
